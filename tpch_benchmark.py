...
class Benchmark:
    ...
    def __alter_tables(self):
        commands = [
            ...
            "CREATE PROCEDURE `refresh_function2`(sf float) BEGIN declare i int default 1; declare iterations int default floor(1500 * sf); declare o_key bigint default 0; START TRANSACTION; while i < iterations do set o_key = (select O_ORDERKEY from ORDERS where O_CUSTKEY = 7801 AND O_ORDERSTATUS = 'O' AND O_TOTALPRICE = total AND O_ORDERDATE = '1996-12-06' AND O_ORDERPRIORITY = '5-LOW' AND O_CLERK = 'Clerk#000000616' AND O_SHIPPRIORITY = 0 AND O_COMMENT = 'ly special requests' LIMIT 1); delete from LINEITEM where L_ORDERKEY = o_key; delete from ORDERS where O_ORDERKEY = o_key; set i = i + 1; end while; COMMIT; END", 
