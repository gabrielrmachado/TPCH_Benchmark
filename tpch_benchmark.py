...
class Benchmark:
    ...
    def __create_tables(self):
        commands = [
            ...
            "CREATE PROCEDURE `refresh_function1` (sf float) BEGIN declare iterations1 int default 0; declare iterations2 int default 0; declare i int default 0; declare j int default 0; set iterations1 = floor(1500 * sf); start transaction; while i < iterations1 do UPDATE ORDERS SET O_ORDERSTATUS = 'O', O_TOTALPRICE = 20000.00, O_ORDERDATE = 1996-12-06, O_ORDERPRIORITY = '5-LOW', O_CLERK = 'Clerk#000000616', O_SHIPPRIORITY = 0, O_COMMENT = 'ly special requests' WHERE O_ORDERKEY= @i; set iterations2 = floor(RAND() * (7-1)+1); while j < iterations2 do UPDATE CUSTOMER SET C_NAME = 'Customer#00000000663', C_ADDRESS = 'IVhzIApeRb ot,c,E', C_NATIONKEY = 12, C_PHONE = '25-989-741-2988', C_ACCTBAL = 711.56, C_MKTSEGMENT = 'BUILDING', C_COMMENT = 'to the even, regular platelets. regular, ironic epitaphs nag e' where C_CUSTKEY = @j; set j = j + 1; end while; set i = i + 1; end while; commit; END" 
        ]
        try:
